...
if (isset($_SESSION['profile_created']) && $_SESSION['profile_created'] === true) {
    header("Location: booking1.php?apartment=LuxuryApartment"); // Or any other villa param
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $PROPERTYNAME = isset($_POST['apartment-name']) ? $_POST['apartment-name'] : '';
    $NAME      = $_POST['full-name'];
    $EMAIL      = $_POST['email'];
    $PHONE     = $_POST['phone'];
    $DATE       = $_POST['date'];

    $stmt = $conn->prepare("INSERT INTO booking1 (PROPERTYNAME,NAME,EMAIL,PHONE,DATE) VALUES (?, ?, ?, ?, ?)");
...
$apartment_name = isset($_GET['apartment']) ? htmlspecialchars($_GET['apartment']) : '';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>Book Your Apartment</title>
...
<div class="booking-container">
    <h2>Book Your Luxury Apartments</h2>
    <form method="POST" action="">
        <label for="apartment-name">Apartment Name:</label>
        <input type="text" id="apartment-name" name="apartment-name" value="<?php echo $apartment_name; ?>" readonly class="form-control">
